...
define('LIST_READLIST', 'chat_readlist_hash'); //已读聊天列表
define('LIST_UNREADLIST', 'chat_unreadlist_hash'); //未读聊天列表
...
public function chat($server, $frame, $data)
{
    ...
    if ($toFd) {
        ...
        $redis->lpush(HASH_READLIST.json_encode( asort([$data['username'],$data['toUsername']])), json_encode($data));
    } else {
        $msg = ['module' => 'app', 'act' => 'chat', 'err' => -1, 'msg' => '用户不在线'];
        $this->send($server, $fd, $msg);
        $redis->lpush(HASH_UNREADLIST.$data['toUsername'], json_encode($data));
    }
...
public function register($server, $frame, $data)
{
    ...
    if ($exists) {
        ...
        $ret = $redis->hsetnx(HASH_USERINFO, $data['username'], json_encode($data));
        $msg['err'] = $ret ? 0 : $ret;
